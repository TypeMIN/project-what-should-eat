...
import { selectRecommendedCandidates } from "@/lib/recommendation";
import { getSupabaseAdmin } from "@/lib/supabase/admin";
import type { Gender, PreferenceResponse } from "@/lib/types";
...
export async function POST(request: Request) {
  ...
  const [participationResult, participantFeedbackResult, comparisonResult, populationFeedbackResult] =
    await Promise.all([
      supabase
  ...
  if (participationResult.error) return apiError("최근 선택 이력을 확인하지 못했습니다.", 500);
  if (participantFeedbackResult.error || comparisonResult.error || populationFeedbackResult.error) {
    return apiError("추천 학습 데이터를 확인하지 못했습니다.", 500);
  }

  const decisionIds = [
    ...new Set((participationResult.data ?? []).map((row) => Number(row.decision_id))),
  ];
  ...
  if (decisionIds.length > 0) {
    const { data: decisions, error: decisionError } = await supabase
      ...
      .in("id", decisionIds);
    if (decisionError) return apiError("최근 선택 이력을 확인하지 못했습니다.", 500);
    ...
    for (const decision of decisions ?? []) {
      ...
      if (decidedAt >= categorySince) {
        const category = decision.category_name.split(">").map((level) => level.trim())[1] || "기타";
  ...
  const populationUserIds = [...new Set(combinedFeedback.map((row) => Number(row.user_id)))];
  const { data: population, error: populationError } = populationUserIds.length === 0
    ? { data: [], error: null }
    : await supabase
        .from("app_users")
        .select("id, birth_year, gender")
        .in("id", populationUserIds);
  if (populationError) return apiError("추천 집단 데이터를 확인하지 못했습니다.", 500);
