...
function buildCategoryVectors(feedback: readonly FeedbackSignal[]) {
  const totals = new Map<number, Map<string, { total: number; count: number }>>();
  for (const item of feedback) {
    if (item.response === "not_visited") continue;
  ...
  return new Map(
    [...totals].map(([userId, categories]) => [
      userId,
      new Map([...categories].map(([category, value]) => [category, value.total / value.count])),
    ]),
  );
}

function cosineSimilarity(left: ReadonlyMap<string, number>, right: ReadonlyMap<string, number>) {
...
function average(values: readonly number[]) {
  return values.length === 0 ? DEFAULT_SCORE : values.reduce((sum, value) => sum + value, 0) / values.length;
}
...
function predictedPreference(
  ...
  if (direct) signals.push({ value: responseValue(direct.response), weight: 5 });
  if (categoryValues.length > 0) {
    signals.push({ value: average(categoryValues), weight: Math.min(3, categoryValues.length) });
  }
  if (comparisonValues.length > 0) {
    signals.push({ value: average(comparisonValues), weight: Math.min(1.5, comparisonValues.length * 0.15) });
  }

  const personalEvidence = userFeedback.length + Math.min(4, comparisonValues.length / 4);
  const userVector = vectors.get(user.id);
  if (userVector && userVector.size >= 2 && personalEvidence < 8) {
    const neighbors = [...vectors]
      .filter(([otherUserId, vector]) => otherUserId !== user.id && vector.has(category))
    ...
    if (neighbors.length > 0) {
      const totalSimilarity = neighbors.reduce((sum, neighbor) => sum + neighbor.similarity, 0);
      const neighborValue = neighbors.reduce(
        (sum, neighbor) => sum + neighbor.value * neighbor.similarity,
        0,
      ) / totalSimilarity;
      signals.push({ value: neighborValue, weight: Math.max(0.2, 1.2 - personalEvidence * 0.15) });
    }
  }

  if (personalEvidence < 5) {
    const populationById = new Map(context.population.map((member) => [member.id, member]));
    ...
    if (demographicValues.length >= 3) {
      signals.push({ value: average(demographicValues), weight: Math.max(0.2, 0.8 - personalEvidence * 0.15) });
    }
    ...
    if (globalValues.length >= 2) {
      signals.push({ value: average(globalValues), weight: Math.max(0.1, 0.45 - personalEvidence * 0.08) });
    }
  }
...
function isStronglyDisliked(
  ...
  const relevant = latestFeedback.filter((item) => participantIds.has(item.userId));
  if (relevant.some((item) => item.placeId === place.id && item.response === "disliked")) return true;
...
export function selectRecommendedCandidates(
  ...
  const participantFeedback = latestFeedback.filter((item) => participantIds.has(item.userId));

  const ranked: RankedCandidate[] = places
    .filter(isMealCandidate)
    .filter((place) => !isStronglyDisliked(place, context.participants, latestFeedback))
    .map((place) => {
      const category = getMajorCategory(place.category);
      const groupPreference = average(
        context.participants.map((participant) => (
          predictedPreference(participant, place, latestFeedback, vectors, context)
        )),
      );
      ...
      return {
        place,
        category,
        score: groupPreference + distanceBonus + random() * 0.02 - (recent ? 2 : 0),
        newPlace: !context.visitedPlaceIds.has(place.id)
          && !participantFeedback.some((item) => item.placeId === place.id),
        rediscovery: !recent
          && participantFeedback.some((item) => item.placeId === place.id && item.response === "liked"),
      };
    })
    .sort((left, right) => right.score - left.score);

  const selected: RankedCandidate[] = [];
  const fresh = ranked.filter((candidate) => !context.recentPlaceIds.has(candidate.place.id));
  ...
  const variety = [...fresh].sort((left, right) => {
    const frequencyDifference = (context.recentCategoryCounts.get(left.category) ?? 0)
  ...
  for (let categoryCap = 3; selected.length < limit && categoryCap <= limit; categoryCap += 1) {
