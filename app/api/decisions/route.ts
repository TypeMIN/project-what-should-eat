...
export async function POST(request: Request) {
  ...
  const { data: decision, error: decisionError } = await supabase
    ...
    .single();
  if (decisionError || !decision) return apiError("결과를 저장하지 못했습니다.", 500);

  const { error: participationError } = await supabase
    .from("meal_decision_participants")
    .insert(participantIds.map((userId) => ({ decision_id: decision.id, user_id: userId })));
...
export async function GET() {
  ...
  const userMap = new Map((users as UserRow[]).map((user) => [Number(user.id), user]));
  const participantsByDecision = new Map<number, DecisionHistory["participants"]>();
