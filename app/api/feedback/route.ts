...
import type { PlaceCandidate, PlaceFeedback, PreferenceResponse } from "@/lib/types";
...
const RESPONSES = new Set<PreferenceResponse>(["liked", "disliked", "not_visited"]);
const FEEDBACK_COLUMNS = "id, decision_id, place_id, place_name, category_name, address_name, road_address_name, place_url, latitude, longitude, response, source, updated_at";
...
export async function GET() {
  ...
  return Response.json({ feedback: (data as FeedbackRow[]).map(toPlaceFeedback) });
}
...
export async function POST(request: Request) {
  ...
  if (!response || !RESPONSES.has(response)) return apiError("평가 항목을 확인해 주세요.");
  ...
  if (hasDecision) {
    ...
    const { data: decision, error: decisionError } = await supabase
      .from("meal_decisions")
      .select("place_id, place_name, category_name, distance_meters, address_name, road_address_name, place_url, latitude, longitude")
      .eq("id", decisionId)
      .maybeSingle();
    if (decisionError || !decision) return apiError("선택한 식당 정보를 찾지 못했습니다.", 404);
    place = {
  ...
  const query = existing
    ? supabase.from("place_feedback").update(values).eq("id", existing.id)
    : supabase.from("place_feedback").insert(values);
  const { data: saved, error: saveError } = await query.select(FEEDBACK_COLUMNS).single();
  if (saveError || !saved) return apiError("평가를 저장하지 못했습니다.", 500);

  return Response.json({ feedback: toPlaceFeedback(saved as FeedbackRow) }, { status: existing ? 200 : 201 });
}
